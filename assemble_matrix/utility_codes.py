...
#   Purpose:   Collection of utility codes for assemblematrix
...
class InpClass():
    def __init__(self, inp):
        ...
        self.crust1 = \
            eval(Config_ini.get('section_raydata', 'crust1'))
        # raymatrix
...
def make_assemble_dir(master_add, target_add, vertex_file, facet_file,
    ...
    if len(mat_files_glob) == 0:
        mat_files_glob = glob.glob(os.path.join(target_add, 'matrixT.*'))
    mat_files_glob.sort()
    ...
    try:
        ...
        new_vertex_dir = os.path.join(target_add, vertex_file)

        cur_facet_dir = os.path.join(all_dirs[0], facet_file)
        new_facet_dir = os.path.join(target_add, facet_file)
    ...
    in_assemble_fio.writelines('%s %s\n' % (hypocen_corr_depth[0], hypocen_corr_depth[1]))
...
def compile_assemblematrix(target_add):
    ...
    shutil.copy('assemblematrix', os.path.join(target_add))
...
def plot_assemble_dtheor(assembled_dir, out_ident):
    ...
    for li in range(0, len(fi)):
        ...
        chiw.append(float(fi[li].split()[3]))
    ...
    plt.savefig(os.path.join(assembled_dir, 'nonzero.png'))

    plt.figure(figsize=(15, 10))
    plt.subplot(2, 1, 1)
    plt.plot(range(1, len(rhs)+1), rhs, lw=3)
    plt.xlabel('#measurement', size=24, weight='bold')
    plt.ylabel('RHS: weighted data [s]', size=24, weight='bold')
    plt.xticks(size=18, weight='bold')
    plt.yticks(size=18, weight='bold')
    plt.ylim(-30, 30)
    plt.subplot(2, 1, 2)
    # import ipdb; ipdb.set_trace()
    plt.plot(range(1, len(rhs)+1), np.array(rhs)*np.array(chiw), lw=3)
    plt.xlabel('#measurement', size=24, weight='bold')
    plt.ylabel('RHS: data [s]', size=24, weight='bold')
    plt.xticks(size=18, weight='bold')
    plt.yticks(size=18, weight='bold')
    plt.ylim(-30,30)
    plt.savefig(os.path.join(assembled_dir, 'rhs.png'))
    ...
    plt.savefig(os.path.join(assembled_dir, 'dtheor.png'))
    # plt.show()
...
def plot_assemble_stations(assembled_dir, out_ident):
    ...
    plt.savefig(os.path.join(assembled_dir, 'number_of_repetition.png'))
    ...
    m.scatter(x, y, s=70, c=stdt, marker="o", edgecolor="none", zorder=10,
            vmin=-7, vmax=7)
    plt.colorbar()
    plt.title('Average dT', size=24, weight='bold')
    plt.savefig(os.path.join(assembled_dir, 'average_dT'))
