      ...
      ! (should be consistent with mpisolvetomo's solvet.h)
...
c      dimension rmsb(NFR)  ! rms noise in bands
      ...
      character*32 chcor(10),chcorrection(4)
...
! initialize
      do i=1,NDIM
        ata(i)=0.      ! diagonal of matrix A'A
      enddo  
     ...
     &      stop 'with hypo added, kount>NMAX; increase dimensions'
        ...
        if(kdatatype.eq.1) then
          ! First find unit vector in ray direction:
           ...
           asparse(kount+3)=-dpar(4)*pz/sv ! shift Up
           do i=1,3
              ja(kount+i)=ilast+i ! ja keep track of column numbers
           end do         
           kount=kount+3        ! nonzero's in this row
        endif
...
!    
!     K.S. 2010/08/26 format change from the below = Guust's bug fix.
...
345       format('inmapc.',i1)
          ...
          do i=1,np
            ...
            amx=max(a,amx)
            write(7,346) (points(j,i),j=1,3),a
346         format(3f8.1,e14.5)
          enddo
          close(7)
