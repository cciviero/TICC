...
c     parameter(NPMAX=100000) ! number of model grid nodes
      ...
      dimension colden(NMAX), scaall(NMAX), scaalltot(NMAX) 
      ...
      INTEGER :: ksmooth_scale, nrden
      REAL :: sca, tmptmp, colvalue
...
c      data ytry_lcurve/1.0, 0.01,.05,.07,.1,.15,.2, .3, .4, .6, .8/
...
!      data ytry_lcurve/1.0, 0.03, .1, .2, .3, .4, .5, .6, .7, .8, .9/
      ...
      ! ksmooth_scale solves the follwoing equation:
      ! A*S^(-1/2)*m' = b in which m' = S^(1/2)*m
      ! S itself is: Sjj = hj*Vj
      ! hj: hitcounts
      ! Vj: volume
      ksmooth_scale = 0
...
!      open(13,file=directory(1:ldr)//'diagnostics.solve')
      ...
      ! open columndensity
      open(17,file=directory(1:ldr)//'columndensity.1.'//ident2)
...
c In the inversion, we use rhs (hence its name)
      ...
      nrow_kntaverage = nrow*kntaverage
...
c============================================================
      ...
      call MPI_BCAST(u,nrowd,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(tvol,NPMAX,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(colden,NMAX,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(scaall,NMAX,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(vscale,1,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(np,1,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      call MPI_BCAST(jlast,11,MPI_REAL,0,MPI_COMM_WORLD,ierr)
      ...
      do irow=kdisp(myid)+1,kdisp(myid)+knt(myid)
        if(.not.outl(irow)) then
          do k=1,ia(irow)
            j=j+1
            jj=ja(j)
            if (ksmooth_scale == 1) then
              sca=100.
              if (jj.le.jlast(4)) then 
                sca=tvol(mod(jj-1,np)+1)*colden(mod(jj-1,np)+1)
                sca=sqrt(sca)
              endif
              scaall(jj) = sca*0.01
              w1(jj)=w1(jj)+(1./scaall(jj))*a(j)*u(irow)
            else
              w1(jj)=w1(jj)+a(j)*u(irow)
            end if
          enddo
...
c gather the w1's from each subprocess and sum them into v
      ...
      call MPI_REDUCE(scaall,scaalltot,NMAX,MPI_REAL,
...
c (no need to add to original v since that v initialized to 0)
      ...
      open(18,file=directory(1:ldr)//'scale_factors.'//ident2)
      do i=1,ncol
        write(18, *) scaalltot(i), v(i)
      enddo
...
c in lsqr we have here: call avpu(ksmooth,m,n,u,v,outl)
        ...
        do ii=1,ncol        ! norm damping of all paramaters incl corr
          i=i+1                   ! true row index
          x=1.0
          if(ii.le.jlast(4)) then
              x=tvol(mod(ii-1,np)+1) ! grid node
              xcol=max(abs(colden(mod(ii-1,np)+1)), 0.0001)
          endif
          u(i)=u(i)+(1./xcol)*x*epsnorm*w1(ii)
        enddo
        ! smoothing constraints within matrix A ?
        if(ksmooth.le.1.and.epssmooth.ne.0.) then
          do ii=1,jlast(4)    ! smoothness damping over model only
            ...
            i0=ii-kk          ! remember shift in model vector
            u(i)=u(i)+tnab(kk)*epssmooth*w1(ii)  ! scale kk using tnab()
            x=tnab(kk)*epssmooth/nb(kk)     
            do j=1,nb(kk)
              u(i)=u(i)-x*w1(iv(j,kk)+i0)
            enddo  
          enddo  
        endif  
...
c       write(6,*) 'Root starts damping ', nrow,ncol,np
        ...
        if(ksmooth.le.1.and.epssmooth.ne.0) then
          ...
          do ii=1,jlast(4)                 ! smoothness damping
            ...
            i0=ii-kk                ! offset    
            w1(ii)=w1(ii)+tnab(kk)*epssmooth*u(i)  ! central node
            x=tnab(kk)*epssmooth/nb(kk)
            do j=1,nb(kk)
              jj=iv(j,kk)+i0
              w1(jj)=w1(jj)-x*u(i)  ! minus neighbours  
            enddo
          enddo
        endif  
...
cc          write(6,*) 'root calls smooth'
           call smooth(sol,v,ncol,epsnorm,epssmooth,nb,iv,ksmooth,jlast)
            ...
            do i=1,ncol
              if (ksmooth_scale == 1) then
                if (scaalltot(i).gt.0.1) then
                  v(i)=sol(i)*(1./scaalltot(i))
                else
                  v(i)=0.
                endif
              else
