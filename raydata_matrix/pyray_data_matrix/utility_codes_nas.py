...
def print_inp(inp, req_band):
    ...
    """
    print '==============INPUT==================='
    print 'Event DIR: %s' % inp.events_dir
    print 'Requested Phase: %s' % inp.phase
    print 'Requested Band: %s' % req_band
    if inp.all_events == True:
        print 'Requested events: ALL'
    else:
        print 'Requested events: %s' % inp.all_events
    if inp.run_raydata:
        print '--------raydata---------'
        print 'INPUT name: %s' % inp.input_file_name_part
        print 'Twinned: %s' % inp.twinned
        print 'Maximum number of arrivals: %s' % inp.max_num_arrival
        print 'Delay wrt the first arrival: %s' % inp.delay_wrt_first_arrival
    ...
    if inp.run_raymatrix:
        print '-------raymatrix--------'
        print 'Vp, Vs, Qs : %s' % inp.vp_vs_Qs
        print 'Kernel Quadrature (KM): %s' % inp.kernel_quad_km
        print 'Vertex file: %s' % inp.vertex_file
        print 'Facet file: %s' % inp.facet_file
    ...
    print '============CRITERIA=============='
    print '%s <= Depth < %s' % (inp.min_depth, inp.max_depth)
    print '%s <= xcorr < %s' % (inp.min_xcorr, inp.max_xcorr)
    print '%s <= epicentral < %s' % (inp.min_epi, inp.max_epi)
    print 'Check clip: %s' % inp.check_clip
    print 'Background model: %s' % inp.bg_model
    print '==============END INPUT==================='
...
def event_filter(inp):
    ...
    print '\n======>> reading the event information and filter them ' \
          '%s <= depth < %s' % (inp.min_depth, inp.max_depth)
    if not os.path.isdir(inp.events_dir):
        print "%s is not a valid directory!" % inp.events_dir
        return False, False
...
def array_station_filter(passed_array, inp):
    ...
    print '\n================================='
    print '#event-station pairs (before): %s' % len(passed_array)
    ...
    if not passed_stas_final.size == 0:
        print '#event-station pairs (after):  %s' % len(passed_stas_final)
        print '================================='
        return passed_stas_final
    else:
        print '#event-station pairs (after):  0'
        print '================================='
        return empty_array
...
def prepare_dir(input_file_name, inp, vertices, facets):
    ...
    :return:
    """
    print '\n======>> prepare output directory at: %s/%s ' \
          % (inp.out_path, input_file_name)
    cur_dir = os.path.abspath(os.curdir)
    if os.path.isdir(os.path.join(
            inp.out_path, '%s_dir' % input_file_name)):
...
def mat2asc_run(input_file_name, inp):
    ...
    """
    cur_dir = os.path.abspath(os.curdir)
    print '\n======>> run mat2asc at  %s/%s_dir' % (inp.out_path, input_file_name)
    os.chdir(os.path.join(inp.out_path, '%s_dir' % input_file_name))
...
def vtk_generator(inp, req_band, len_dirs):
    ...
    for nj in range(len_dirs):
        ...
        print '------> create VTK file at %s/%s_dir' % (inp.out_path, input_file_name)
