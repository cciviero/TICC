...
for i in range(len(ls_matout)):
    ...
    if np.size(rayout_array_tmp[(rayout_array_tmp[:, 12].astype(float) > 105.0), 12]) != 0:
        color = 'r'
        print '\n%s/%s' % (i+1, len(ls_matout))
        print '%s' % ls_matout[i]
        print rayout_array_tmp[rayout_array_tmp[:, 12].astype(float) > 105.0, 1]
    elif np.size(rayout_array_tmp[(rayout_array_tmp[:, 12].astype(float) < 95.0), 12]) != 0:
        color = 'r'
        print '\n%s/%s' % (i+1, len(ls_matout))
        print '%s' % ls_matout[i]
        print rayout_array_tmp[rayout_array_tmp[:, 12].astype(float) < 95.0, 1]
    else:
    ...
    plt.plot(np.linspace(counter, counter+1,
                         len(rayout_array_tmp[:, 12].astype(float))),
             rayout_array_tmp[:, 12].astype(float) - 100.0, '.', c=color)
