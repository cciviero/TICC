...
for i in range(len(ls_npy)):
    ...
    histo, ran = np.histogram(arr_npy[:, 29].astype(float), bins=bins)
    hists.append(histo)
    rans.append(ran)
    if plot_hist:
        plt.ion()
...
for i in range(len(all_num_depth_sorted)-1, -1, -1):
    select_dp_1 = arr_npy[float(all_num_depth_sorted[i][1]) <=
                          arr_npy[:, 29].astype(float)]
    select_dp_2 = select_dp_1[select_dp_1[:, 29].astype(float) <
...
#    print ls_npy[i]
#    arr_npy = np.load(ls_npy[i])
#    hist, ran = np.histogram(arr_npy[:, 29].astype(float),
