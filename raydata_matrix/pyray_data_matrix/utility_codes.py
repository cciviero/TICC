    ...
    if inp.sigma_fix:
        for indx, cc in enumerate(rd_output[:, 6]):
            if eval(cc) < inp.sigma_cc:
                rd_output[indx, 9] = rd_output[indx, 9].astype(float) * inp.sigma_factor
...
def array_station_filter(passed_array, inp):
    ...
    pass_stas_corr_1 = passed_array[
        passed_array[:, 6].astype(float) >= inp.min_xcorr]
    if not pass_stas_corr_1.size == 0:
        pass_stas_final = pass_stas_corr_1[
            pass_stas_corr_1[:, 6].astype(float) < inp.max_xcorr]
    else:
        return empty_array
    # --------------- EPICENTRAL ---------------
    passed_stas_epi_1 = pass_stas_final[
        pass_stas_final[:, 4].astype(float) >= inp.min_epi]
    if not passed_stas_epi_1.size == 0:
        passed_stas_final = passed_stas_epi_1[
            passed_stas_epi_1[:, 4].astype(float) < inp.max_epi]
    else:
        return empty_array
    # --------------- CHECK CLIPS ---------------
    if inp.check_clip:
...
def check_selection(filt_array, inp, output_dir):
    ...
    plt.hlines(inp.min_xcorr, np.min(filt_array[:, 2].astype(float)),
               np.max(filt_array[:, 2].astype(float)), 'k',
               linestyles='--')
    plt.hlines(inp.max_xcorr, np.min(filt_array[:, 2].astype(float)),
               np.max(filt_array[:, 2].astype(float)), 'k',
               linestyles='--')
    ...
    plt.xticks(size='large', weight='bold')
    plt.yticks(size='large', weight='bold')
    plt.hlines(inp.min_xcorr, np.min(filt_array[:, 3].astype(float)),
               np.max(filt_array[:, 3].astype(float)), 'k',
               linestyles='--')
    plt.hlines(inp.max_xcorr, np.min(filt_array[:, 3].astype(float)),
               np.max(filt_array[:, 3].astype(float)), 'k',
               linestyles='--')
    ...
    #py2mat(filt_array[:, 2].astype(float), 'stlat', 'RESULTS/stlat')
    #py2mat(filt_array[:, 3].astype(float), 'stlon', 'RESULTS/stlon')
    #py2mat(filt_array[:, 4].astype(float), 'epi_dist',
    #       'RESULTS/epi_dist')
    #py2mat(filt_array[:, 6].astype(float), 'xcorr', 'RESULTS/xcorr')
...
def compile_raydata_raymatrix(inp):
    ...
    if inp.crust_corr == 'crust1':
        cprint('utility_codes.py', '[compile_raydata_raymatrix]', bc.bmagenta, 'CRUST1.0')
    ...
    if inp.run_raymatrix:
        cprint('utility_codes.py', '[compile_raydata_raymatrix]', bc.bmagenta, 'Compiling raymatrix.f')
        os.chdir(os.path.join(cur_dir, 'src_raydata_raymatrix', 'raymatrix_src'))
        os_sys = os.system('./make')
        if not os_sys == 0:
            cprint('utility_codes.py', '[compile_raydata_raymatrix]',
                   bc.red, 'ERROR: make cannot be compiled!')
...
def raydata_input_generator(filt_array, input_file_name, req_band, inp,
    ...
    if inp.phase == 'Pdiff':
        geocen_lat_sta = geocen_array(filt_array[:, 2].astype(float))
        geocen_lat_evt = geocen_array(filt_array[:, 27].astype(float))
    ...
    for sta in filt_array:
        ...
        counter += 1
        netid = sta[5].decode("utf-8").split('.')[0]
        if not inp.plot_statistics:
            # XXX the decode part is a bit unnecessary because we should be able ton convert it 
            # earlier on from bytes to string
            # if decode is not there we get an error message that this is not a string but byte
            staid = sta[5].decode("utf-8").split('.')[1]
        else:
            staid = '%07i' % counter
        if not "x00" in sta:
            chaid = sta[5].decode("utf-8").split('.')[3]
        else:
            chaid = sta[5].decode("utf-8").split('.')[4]
        # import ipdb; ipdb.set_trace()
        first_line = '%s %s %s %s     %s   %s %s    %s    %s    %s    %s  %s   %s 1 0 0\n' \
                     % (sta[24], sta[25], sta[26], sta[1].decode("utf-8"), staid, netid, chaid,
                        sta[27], sta[28], sta[29], sta[2].decode("utf-8"), sta[3].decode("utf-8"), sta[22])
        ...
        if inp.phase == 'Pdiff':
            ...
            tB_mfi = sta[17].decode("utf-8")
        if not tB_mfi:
            # print "Do not include %s.%s.%s -- dist: %s!" \
            #       % (netid, staid, chaid, sta[4])
            continue
        third_line = '    %s  %s  %s  %s   %s   %s\n' \
                     % (sta[7].decode("utf-8"), sta[9].decode("utf-8"), sta[6].decode("utf-8"),
                        int(req_band.split('band')[1]), sta[18].decode("utf-8"), tB_mfi)
...
def prepare_dir(input_file_name, inp, vertices, facets):
    ...
    if os.path.isdir(os.path.join(inp.out_path, '%s_dir' % input_file_name)):
        answer = input("Directory already exists! Remove [R] or Exit [E]?")
        if answer == 'R':
            shutil.rmtree(os.path.join(inp.out_path, '%s_dir' % input_file_name))
        else:
            sys.exit('Exit. Rename move previous folder or delete manually.')
    ...
    files_glob = os.path.join(os.path.curdir, 'src_raydata_raymatrix', 'files')
...
def raydata_ccorr_reader(filt_array, input_file_name, corr_io_list,
    ...
    grp_ccorr_arr_unique = np.unique(ccorr_arr[:, 1].astype(int))
    indx = []
    ...
    len_ccorr_arr_0 = len(ccorr_arr[ccorr_arr[:, 0].astype(int)
                                    == kd_avail[0]])
    ...
    for i in range(len(kd_avail)):
        ...
        tar_ccorr_arr = ccorr_arr[ccorr_arr[:, 0].astype(int) ==
                                  kd_avail[i]]
        ...
        if not divi:
            if kd_avail[i] == 1:
                ...
                if corr_io_list[0] == 1:
                    t_ellip += tar_ccorr_arr[:, 8].astype(float)
            if corr_io_list[1] == 1:
                t_cc += tar_ccorr_arr[:, 9].astype(float)
            if corr_io_list[2] == 1:
                t_elev += tar_ccorr_arr[:, 10].astype(float)
        else:
            if kd_avail[i] == 1:
    ...
    sub_ccorr_arr = ccorr_arr[ccorr_arr[:, 0].astype(int) == kd_avail[0]]
    t_corr = t_ellip + t_cc + t_elev
    t_final = sub_ccorr_arr[:, 14].astype(float) - \
              (sub_ccorr_arr[:,  13].astype(float) + t_corr)
    ...
    plt.figure()
    plt.subplot(2, 1, 1)
    plt.scatter(filt_array[:, 2].astype(float), filt_array[:, 34],
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 1, 2)
    plt.scatter(filt_array[:, 3].astype(float), filt_array[:, 34],
    ...
    plt.figure()
    plt.subplot(2, 3, 1)
    plt.scatter(filt_array[:, 2].astype(float), filt_array[:, 31], c='r',
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 2)
    plt.scatter(filt_array[:, 2].astype(float), filt_array[:, 32], c='r',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Latitude', size='large', weight='bold')
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 3)
    plt.scatter(filt_array[:, 2].astype(float), filt_array[:, 33], c='r',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Latitude', size='large', weight='bold')
    ...
    plt.subplot(2, 3, 4)
    plt.scatter(filt_array[:, 3].astype(float), filt_array[:, 31], c='b',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Longitude', size='large', weight='bold')
    plt.ylabel('Ellipticity Correction', size='large', weight='bold')
    plt.xticks(size='large', weight='bold')
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 5)
    plt.scatter(filt_array[:, 3].astype(float), filt_array[:, 32], c='b',
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 6)
    plt.scatter(filt_array[:, 3].astype(float), filt_array[:, 33], c='b',
                edgecolors='b', alpha=alpha_plt)
    plt.xlabel('Longitude', size='large', weight='bold')
    plt.ylabel('Topography Correction', size='large', weight='bold')
    plt.xticks(size='large', weight='bold')
    ...
    plt.figure()
    plt.scatter(filt_array[:, 2].astype(float), filt_array[:, 35], c='b',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('latitude', size='large', weight='bold')
...
def plot_stas_proj(filt_array_corr, inp, plot_results=True, indx=35):
    ...
    if plot_results:
        ...
        x, y = mymap(filt_array_corr[:, 3].astype(float),
                     filt_array_corr[:, 2].astype(float))
        x_k, y_k = mymap(filt_black[:, 3].astype(float),
                         filt_black[:, 2].astype(float))
    ...
    if plot_results:
        mymap.scatter(x, y, c=c_plot.astype(float),
                      edgecolor='none',
