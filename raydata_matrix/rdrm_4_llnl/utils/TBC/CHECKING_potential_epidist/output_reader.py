...
def array_station_filter(passed_array, min_xcorr=-100, max_xcorr=100,
    ...
    pass_stas_corr_1 = passed_array[
        passed_array[:, 4].astype(np.float) >= min_epi]
    if not pass_stas_corr_1.size == 0:
        pass_stas_final = pass_stas_corr_1[
            pass_stas_corr_1[:, 4].astype(np.float) < max_epi]
    ...
    #    pass_stas_final[:, 4].astype(np.float) >= min_epi]
    #if not passed_stas_epi_1.size == 0:
    #    passed_stas_final = passed_stas_epi_1[
    #        passed_stas_epi_1[:, 4].astype(np.float) < max_epi]
...
def check_selection(filt_array, min_xcorr, max_xcorr, min_epi, max_epi,
    ...
    plt.hlines(min_xcorr, np.min(filt_array[:, 2].astype(np.float)),
               np.max(filt_array[:, 2].astype(np.float)), 'k',
               linestyles='--')
    plt.hlines(max_xcorr, np.min(filt_array[:, 2].astype(np.float)),
               np.max(filt_array[:, 2].astype(np.float)), 'k',
               linestyles='--')
    ...
    plt.xticks(size='large', weight='bold')
    plt.yticks(size='large', weight='bold')
    plt.hlines(min_xcorr, np.min(filt_array[:, 3].astype(np.float)),
               np.max(filt_array[:, 3].astype(np.float)), 'k',
               linestyles='--')
    plt.hlines(max_xcorr, np.min(filt_array[:, 3].astype(np.float)),
               np.max(filt_array[:, 3].astype(np.float)), 'k',
               linestyles='--')
    ...
    import py2mat_mod
    py2mat_mod.py2mat(filt_array[:, 2].astype(np.float), 'stlat', 'stlat')
    py2mat_mod.py2mat(filt_array[:, 3].astype(np.float), 'stlon', 'stlon')
    py2mat_mod.py2mat(filt_array[:, 4].astype(np.float), 'epi_dist',
                      'epi_dist')
    py2mat_mod.py2mat(filt_array[:, 6].astype(np.float), 'xcorr', 'xcorr')
...
def raydata_ccorr_reader(filt_array, input_file_name, corr_io_list):
    ...
    t_corr = np.zeros(
        len(ccorr_arr[ccorr_arr[:, 0].astype(np.int) == kd_avail[0]]))
    t_ellip = np.zeros(
        len(ccorr_arr[ccorr_arr[:, 0].astype(np.int) == kd_avail[0]]))
    t_cc = np.zeros(
        len(ccorr_arr[ccorr_arr[:, 0].astype(np.int) == kd_avail[0]]))
    ...
    for i in range(len(kd_avail)):
        tar_ccorr_arr = ccorr_arr[
            ccorr_arr[:, 0].astype(np.int) == kd_avail[i]]
        ...
        if not divi:
            if kd_avail[i] == 1:
                if corr_io_list[0] == 1:
                    t_ellip += tar_ccorr_arr[:, 6].astype(np.float)
            if corr_io_list[1] == 1:
                t_cc += tar_ccorr_arr[:, 7].astype(np.float)
            if corr_io_list[2] == 1:
                t_elev += tar_ccorr_arr[:, 8].astype(np.float)
        else:
            if kd_avail[i] == 1:
                ...
                if corr_io_list[0] == 1:
                    ...
                    for j in range(0, len(tar_ccorr_arr), divi):
                        e_corrector = np.append(
                            e_corrector,
                            np.sum(tar_ccorr_arr[j:j+divi, 6].astype(np.float)))
                    t_ellip += e_corrector
            if corr_io_list[1] == 1:
                ...
                for j in range(0, len(tar_ccorr_arr), divi):
                    cc_corrector = np.append(
                        cc_corrector,
                        np.sum(tar_ccorr_arr[j:j+divi, 7].astype(np.float)))
                t_cc += cc_corrector
            if corr_io_list[2] == 1:
                ...
                for j in range(0, len(tar_ccorr_arr), divi):
                    el_corrector = np.append(
                        el_corrector,
                        np.sum(tar_ccorr_arr[j:j+divi, 8].astype(np.float)))
                t_elev += el_corrector
    ...
    plt.figure()
    plt.subplot(2, 1, 1)
    plt.scatter(filt_array[:, 2].astype(np.float), t_corr,
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 1, 2)
    plt.scatter(filt_array[:, 3].astype(np.float), t_corr,
    ...
    plt.figure()
    plt.subplot(2, 3, 1)
    plt.scatter(filt_array[:, 2].astype(np.float), t_ellip, c='r',
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 2)
    plt.scatter(filt_array[:, 2].astype(np.float), t_cc, c='r',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Latitude', size='large', weight='bold')
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 3)
    plt.scatter(filt_array[:, 2].astype(np.float), t_elev, c='r',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Latitude', size='large', weight='bold')
    ...
    plt.subplot(2, 3, 4)
    plt.scatter(filt_array[:, 3].astype(np.float), t_ellip, c='b',
                edgecolors='none', alpha=alpha_plt)
    plt.xlabel('Longitude', size='large', weight='bold')
    plt.ylabel('Ellipticity Correction', size='large', weight='bold')
    plt.xticks(size='large', weight='bold')
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 5)
    plt.scatter(filt_array[:, 3].astype(np.float), t_cc, c='b',
    ...
    plt.yticks(size='large', weight='bold')
    plt.subplot(2, 3, 6)
    plt.scatter(filt_array[:, 3].astype(np.float), t_elev, c='b',
                edgecolors='b', alpha=alpha_plt)
    plt.xlabel('Longitude', size='large', weight='bold')
    plt.ylabel('Topography Correction', size='large', weight='bold')
    plt.xticks(size='large', weight='bold')
