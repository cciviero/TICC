function [p,t]=distmeshnd_hvar(fdist,fh,h,box,fix,varargin)
%DISTMESHND_HVAR: like DISTMESHND but varargin feeds the
% h-function rather than the d-function. Necessitates that
% d-function be given analytically as inline function.
% Original lines are marked %!%
% K.S. 2006/12/15
% 
% Note: distmeshnd_hvar saves its results every 5 iters
%  if mod(count,5)==0, save dmeshtmp p t; end  
% which means it can be interrupted at any time and
% the results can be reloaded for the final triangulation.
% K.S.
%
...
disp(sprintf('N0=%d vertices before rejection pass',size(p,1)));
...
disp(sprintf('N =%d vertices in iterative refinement',N));
...
while 1
  ...
  disp(sprintf('Iteration # %d, maxdp = %f, break if maxdp <= %f',...
	       count, maxdp,ptol*h)) %%  
  % intermediate saves  %%
  if mod(count,5)==0
    save dmeshtmp p t
...
% Draw final mesh
simpplot(p,t,'p(:,2)>0');
title(['Final mesh: Retriangulation #',int2str(count)])
drawnow
