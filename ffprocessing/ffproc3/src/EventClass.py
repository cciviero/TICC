...
def read_events_dmt(inp_ffproc):
    ...
    for event_rd in all_events_rd:
        try:
            # Pickle file is generated for each event separately and should
            # contain all the information that we need at this step.
            ev_load = open(os.path.join(event_rd, 'info', 'event.pkl'), 'rb')
            ev_pkl = pickle.load(ev_load)
            ev_load.close()
        except Exception as exc:
            cprint('EventClass.py', '[PICKLE]', bc.lred,
        ...
        try:
            ...
            cprint('EventClass.py', '[PICKLE]', bc.orange,
                   f'event.pkl in {event_rd} does not contain moment tensor information - {exc}. Checking SCARDEC.')
            
            event_name = '%s%02d%02d_%02d%02d%02d' % (ev_date.year, ev_date.month,
                                             ev_date.day, ev_date.hour, ev_date.minute, ev_date.second)
            path_2_scardec_ev = glob.glob(os.path.join(inp_ffproc.stf_scardec_db, '*'+event_name+'*'))
            
            if len(path_2_scardec_ev) == 0:
            # if not found scardec event then go back to archive STF
                cprint('EventClass.py', '[PICKLE]', bc.lred, 'No corresponding SCARDEC moment tensor. Skipping this event.')
                continue
            else:
