...
# -------------------------------------------------------------------
#   Filename:  plot_ffproc_output.py
#   Purpose:   
...
# input_path = '/data/maria/UPFLOW/TICC/measurement_check/pyffproc_2022-12-01_scardec_prem'
# input_path = '/data/maria/global_tomography/measurements/NorthAtlantic_UCL_UPFLOW_2016-2022'
# input_path = '/data/maria/global_tomography/measurements/PICASSO_2009-2013'
input_path = '/data/maria/global_tomography/measurements/IberArray_2009-2013'
# input_path = '/data/maria/global_tomography/measurements/SpanishDigitalSeismicNetwork_2009-2012'

# path to event_list_pickle in EVENTS-INFO folder
# original_path = '/mnt/home_geo/mariat/Codes/_INPUT_/neic_2016_2020_all'
# # original_path = '/data/maria/global_tomography/data_NorthAtlantic/NorthAtlantic_2016-2022'
# original_path = '/data/maria/global_tomography/data_NorthAtlantic/PICASSO_2009-2013'
original_path = '/data/maria/global_tomography/data_NorthAtlantic/IberArray_2009-2013'
# original_path = '/data/maria/global_tomography/data_NorthAtlantic/SpanishDigitalSeismicNetwork_2009-2012'
...
def read_ampstt(path, event_name, band):
    ...
    dt = np.dtype([('idx', int), ('grp', int), ('stlat', float), ('stlon', float), ('epd', float),
                   ...
                   ('ts_step1', float), ('ts_step2', float), ('cc_step1', float), ('cc_step2', float), 
                   ('clip_step1', int), ('clip_step2', int)])
    pandas_dt = {'idx': int, 'grp': int , 'stlat': float , 'stlon': float , 'epd': float ,
                   'name':'S10' , 'cc': float , 'tobs': np.float64, 'dt': float , 'sigma_t': float ,
                   'N/A_1':'S10' , 'N/A_2':'S10' , 'f_amp': float , 'sigma_a': float , 'N/A_3':'S10' ,
                   'N/A_4':'S10' , 'tB_smgr': float , 'tB_mfi': float ,
                    'winlen ': float ,  'clip ': int ,  'N/A_5 ': 'S10' ,  'snr ': float }
    names = ('idx', 'grp', 'stlat', 'stlon', 'epd',
                   'name', 'cc', 'tobs', 'dt', 'sigma_t',
                   'N/A_1', 'N/A_2', 'f_amp', 'sigma_a', 'N/A_3',
                   'N/A_4', 'tB_smgr', 'tB_mfi',
                   'winlen', 'clip', 'N/A_5', 'snr', 'ts_step1','ts_step2','cc_step1','cc_step2','clip_step1','clip_step2')
    ampstt_dir = glob.glob(os.path.join(path, event_name, 'outfiles', 'ffproc.ampstt.band%02d' % band))[0]
    try:
        # import ipdb; ipdb.set_trace()
        station_list = np.loadtxt(ampstt_dir, dtype=dt, ndmin=1)
        df = pd.read_table(ampstt_dir, delim_whitespace=True, comment='#', names=names)
...
for i, value in enumerate(bands_sec):
    ...
    for j, event in enumerate(check_all_events):
        # import ipdb; ipdb.set_trace()
        info_pkl = read_event_pkl(original_path)
        try:
            indx = [i for i, ev in enumerate(info_pkl) if ev['event_id'] == event][0]
        except Exception as exp:
            # this can happen if one event was not processed in pyffproc but is actually in the
            # event pickle file
            continue
        mag = info_pkl[indx]['magnitude']

        all_ev = len(check_all_events)
        all_bd = len(bands_sec)
        print(f'Band {i+1} of {all_bd}| {event} | {j+1} of {all_ev}')
...
if export_all:
    print('Exporting bandwise into txt file. \n')

    for idx, band in enumerate(bands):

        c = [final_cc[idx], final_ts[idx], final_amp[idx], final_mag[idx], final_azi[idx], final_name[idx], final_snr[idx], final_stla[idx], final_stlo[idx]] 
        outfile_fio = open(os.path.join(output_dir, 'B%s_output_%s_%s.txt' % (band, cc_factor, ts_factor)), 'w')
        outfile_fio.writelines('# CC \t | \t time shift \t | \t amplitude \t | \t magnitude \t | \t azimuth \t | \t station name \t | \t SNR \t | \t sta lat \t | \t sta lon\n')
        for x in zip(*c):
            outfile_fio.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".format(*x))
...
if plot_cc_hist:
    ...
    for idx, band in enumerate(bands):
        # import ipdb; ipdb.set_trace()
        y, binEdges = np.histogram(final_cc[idx].astype(float), bins=bins_bin, range=bins_range)
...
if plot_ts_hist:
    ...
    for idx, band in enumerate(bands):
        # import ipdb; ipdb.set_trace()
        y, binEdges = np.histogram(final_ts[idx].astype(float), bins=bins_bin, range=bins_range)
        bin_centers = 0.5 * (binEdges[1:] + binEdges[:-1])
...
# -----------------------------------------------------------------------
# plot ts histogram by station
if plot_station_wise:
    print('plotting ts station wise histogram')

    # all_stations = set(final_name)
    
    all_stations = np.array([])
    for name_list in final_name:
        all_stations = np.append(all_stations, name_list)
    ...
    for jdx, sta in enumerate(all_stations):
        ...
        for idx, band in enumerate(bands):
            #import ipdb; ipdb.set_trace()
            select_indx = np.where(final_name[idx] == sta)
            idx_list = select_indx[0].tolist()
            # print(band, idx_list)
            print(f'{sta.decode("utf-8")} | Band {idx+1}')

            try:
                y, binEdges = np.histogram(final_ts[idx][idx_list].astype(float), bins=bins_bin, range=bins_range)
                bin_centers = 0.5 * (binEdges[1:] + binEdges[:-1])

                # import ipdb; ipdb.set_trace()
                plt.plot(bin_centers, y, color=colors[idx], lw=line_width[idx],
                        label='B0%s - %2.2fs: %s measr.' % (i+1, band,
                                                                len(final_ts[idx][idx_list])))
            except Exception as exp:
                import ipdb; ipdb.set_trace()
                pass
        ...
        # import ipdb; ipdb.set_trace()
        sta_name =  sta.decode("utf-8").split('.')
        plt.savefig(output_dir + '%s.%s_%s_%s_%s_%s.png' % (sta_name[0],sta_name[1], save_label, cc_factor, ts_factor, clip_factor), dpi=500)
        print('\n')
...
if plot_piechart:
    ...
    for band in final_name:
        ...
        for station in unique_stations:
            try:
                net = station.astype('str').split('.')[0]
    ...
    labels = sort_count[:, 0]
    sizes = sort_count[:, 1].astype(float) * 100 / total_number
...
if plot_snr_hist:
    ...
    for idx, band in enumerate(bands):
        ...
        try:
        # import ipdb; ipdb.set_trace()
            y, binEdges = np.histogram(final_snr[idx].astype(float), bins=bins_bin, range=bins_range)
            bin_centers = 0.5 * (binEdges[1:] + binEdges[:-1])
