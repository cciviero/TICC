...
class InpFFproc:
    def __init__(self, inp_ffproc_file):
        ...
        self.gcmt_cat_path = eval(config_ini.get('section_moment_tensors', 'gcmt_cat_path'))
        ...
        # ---------------section obs
        self.sta_info = eval(config_ini.get('section_obs', 'sta_info'))
        try:
            temp = eval(config_ini.get('section_obs', "station_keys"))
            stream = open(temp, "r")
            self.station_keys = yaml.safe_load(stream)
            # self.station_keys = config_ini.get('section_obs', 'station_keys')
        except Exception as exp:
            cprint('InpFFproc.py', '[ERROR]', bc.orange, f"{exp}\nContinue withouth station_keys in [section_obs]. That might cause problems later if you are working with OBS data.")
        ...
        self.skew_correction = eval(config_ini.get('section_obs', 'skew_correction'))
