...
from src.STFclass import read_scardec_header
...
def read_events(inp_ffproc):
    ...
    if inp_ffproc.evproc_mode == 'read':
        ...
        eventcl = read_events_obs(inp_ffproc)
    else:
...
def read_events_dmt(inp):
    ...
    try:
        # evs_pkl -->> all events; ev_pkl -->> one event
        evs_load = open(inp.evproc_path, "rb")
        evs_pkl = pickle.load(evs_load)
        evs_load.close()
    except Exception as exc:
        print(exc)
        cprint(f'EventClass.py', '[PICKLE]', bc.lred,
               '{inp.evproc_path} does not exist or cannot be opened.')
        sys.exit("EXIT!")

    for ev_pkl in evs_pkl:
        # import ipdb; ipdb.set_trace()
        ev_date = ev_pkl['datetime']
        # import ipdb; ipdb.set_trace()
        try:
            ...
            cprint('EventClass.py', '[PICKLE]', bc.orange,
                   f'Event {ev_date} in event.pkl does not contain moment tensor information. ERROR: {exc}. Checking SCARDEC instead.')
            
            # event_name = '%s%02d%02d_%02d%02d%02d' % (ev_date.year, ev_date.month,
            #                                  ev_date.day, ev_date.hour, ev_date.minute, ev_date.second)
            # XXX make smarter with actually making sure not more than 5 sec difference to event time XXX
            event_name = '%s%02d%02d_%02d%02d*' % (ev_date.year, ev_date.month, ev_date.day, ev_date.hour, ev_date.minute)
            
            path_2_scardec_ev = glob.glob(os.path.join(inp.stf_scardec_db, '*'+event_name+'*'))
            # path_2_scardec_ev = []
            
            if len(path_2_scardec_ev) == 1:
                path_2_scardec_stf = glob.glob(os.path.join(path_2_scardec_ev[0], 'fctoptsource_*'))[0]
                scardec_cat_info = read_scardec_header(path_2_scardec_stf)

                # moment_1 =  [Mrr, Mtt, Mpp, Mrt, Mrp, Mtp]
                mrr = scardec_cat_info[13][0]
                mtt = scardec_cat_info[13][1]
                mpp = scardec_cat_info[13][2]
                mrt = scardec_cat_info[13][3]
                mrp = scardec_cat_info[13][4]
                mtp = scardec_cat_info[13][5]

                # print('direct comparison')
                # mrr_2, mtt_2, mpp_2, mrt_2, mrp_2, mtp_2 = get_gcmt_eq_metadata(inp, ev_date)
                # print(mrr, mrr_2, abs((mrr-mrr_2)/np.mean([mrr,mrr_2])))
                # print(mtt, mtt_2, abs(mtt-mtt_2)/np.mean([mtt,mtt_2]))
                # print(mpp, mpp_2, abs((mpp-mpp_2)/np.mean([mpp,mpp_2])))
                             # , mrt-mrt_2, mrp-mrp_2, mtp-mtp_2)
            
            else:
                # if not found scardec event then go back to archive STF or 
                # download with obspy the moment tensor informaiton
                cprint('EventClass.py', '[PICKLE]', bc.orange, 'No corresponding SCARDEC moment tensor. Checking GCMT catalog.')
                mrr, mtt, mpp, mrt, mrp, mtp = get_gcmt_eq_metadata(inp, ev_date)
                
                if not mrr:
                    cprint('EventClass.py', '[PICKLE]', bc.red,
                    f'FFprocessing does not find moment tensor informaiton for {ev_date}. Continue to next event.')
                    continue 
                # import ipdb; ipdb.set_trace()

        ev_name = ev_pkl['event_id']

        scalar_moment_magnitude = np.sqrt(
                    np.power(float(mrr), 2) +
                    np.power(float(mtt), 2) +
                    np.power(float(mpp), 2) +
                    np.power(float(mrt), 2) +
...
# ------------------ get_fdsn_eq_metadata ---------------------------
 

def get_gcmt_eq_metadata(inp, ev_date):
    
    mon_name = ev_date.strftime("%b").lower()
    read_path = glob.glob(os.path.join(inp.gcmt_cat_path, f'{ev_date.year}', f'{mon_name}*.ndk'))
    if len(read_path) == 1:
        cat = obspy.read_events(read_path[0])
    else:
        return None, None, None, None, None, None
    
    # import ipdb; ipdb.set_trace()
    cat_filt = cat.filter(f"time > {ev_date-5} ", f"time < {ev_date+5}")
    if cat_filt.count() == 0:
        return None, None, None, None, None, None
    
    fm = cat_filt.events[0].focal_mechanisms[0].moment_tensor.tensor
    mrr = fm.m_rr
    mtt = fm.m_tt
    mpp = fm.m_pp
    mrt = fm.m_rt
...
def read_events_obs(inp_ffproc):
    ...
    ev_load = open(os.path.join(inp_ffproc.evproc_path, 'EVENTS-INFO', 'event_list_pickle'), 'rb')
    ...
    for ev_pkl in ev_all_pkl:
        ...
        try:
            ...
            cprint('EventClass.py', '[read_events_obs]', bc.orange,
                   'event_list_pickle for %s does not contain moment tensor information - %s. CONTINUE!'
                   % (ev_pkl['datetime'], exc))
            continue
