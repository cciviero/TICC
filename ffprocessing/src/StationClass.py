...
def read_data_header(inp_ffproc, all_events, ev):
    ...
    elif inp_ffproc.real_mode == 'read_ami':
        station_cl = read_data_header_ami(inp_ffproc=inp_ffproc,
                                          all_events=all_events,
                                          ev=ev)
    elif inp_ffproc.real_mode == 'read_obs':
        station_cl = read_data_header_obs(inp_ffproc,
                                          all_events,
                                          ev)
    elif inp_ffproc.real_mode == 'read_repo':
...
def read_data_header_cat(inp_ffproc, all_events, ev):
    ...
    for cha in inp_ffproc.ph_channel:
        ...
        for l_sta in list_stas:
            # import ipdb; ipdb.set_trace()
            # def addsta(self, real_path, inv_path, name, lat, lon, dp, el, evla, evlo, obs_start, obs_end, skew):
            station_cl.addsta(l_sta[1], None, l_sta[2], l_sta[3], l_sta[4],
                              l_sta[5], l_sta[6], l_sta[7], l_sta[8], None, None, None)
...
def read_data_header_obs(inp, all_events, ev):
    """
    Create one station class for land and obs stations (when generating two different classes it makes things a bit cumbersome later)
    """
    
    station_cl = StationClass()
    sta_info = np.loadtxt(inp.sta_info, comments="#", dtype=object)
    sta_info = sta_info.reshape(-1, 11)
    
    for station in sta_info:
        # if inp.station_keys[sta[7]]["call_key"].lower() == "dmt":

        # real_path, name, lat, lon, dp, el, evla, evlo
        
        # 0-NR 1-lat 2-lon  3-depth 4-elevation 5-station_code 6-station_name 7-key
        # no network information in the station info list
        lat = station[1]
        lon = station[2]
        dp = station[3]
        el = station[4]
        ...
        sta_starttime = station[8]
        sta_endtime = station[9]
        sta_skew = station[10]

        # keeping it empty for now and put the proper network code in later
        net = inp.station_keys[station[7]]["network"]
        loc = inp.station_keys[station[7]]["location"]
        
        path = inp.station_keys[station[7]]["path_structure"]
        resp_path = inp.station_keys[station[7]]["response_seismometer"]

        master_path = inp.station_keys[station[7]]["master_path"]
        # import ipdb; ipdb.set_trace()
        name = f'{net}.{sta}.{loc}.{inp.ph_channel[0]}'
        cha = inp.ph_channel[0]
        evla = all_events.lat[ev]
        evlo= all_events.lon[ev]
        year= all_events.datetime[ev].year
        month=f'{all_events.datetime[ev].month:02d}'
        day=f'{all_events.datetime[ev].day:02d}'
        try:
            real_path = glob.glob(eval(path))[0]
            # import ipdb; ipdb.set_trace()
            inv_path = glob.glob(eval(resp_path))[0]
        except Exception:
            continue
        station_cl.addsta(
            real_path, inv_path, name, lat, lon, dp, el, evla, 
            evlo, sta_starttime, sta_endtime, sta_skew)
            # name, code, sta_type, net, loc, lat, lon, dp, el, path, master

    cprint(
        "StationClass.read_data_header_obs",
        "[INPUT]",
        bc.yellow,
        f"Stations added to Station Class | Total number of stations: {len(station_cl)}",
    )

    if len(station_cl.name) == 0:
        cprint(
            "StationClass.read_data_header_obs", "[INPUT]", bc.red, f"Station Class EMPTY."
        )
        return None
        ...
        for sta in sta_info:
            try:
                ...
                dist_deg = locations2degrees(all_events.geocen_lat[ev],
                                             ...
                                             geocen_stla,
                                             float(sta[3]))
                # print(all_events.geocen_lat[ev],all_events.lon[ev],geocen_stla,float(sta[3]), dist_deg)
    ...
    for cha in range(len(all_stations.name)):
        ...
        try:
            ...
            if not len(tr.data) == acc_dur:
                cprint('StationClass.py', '[ERROR] [DATA]', bc.dred,
                       "Length of the data: %s vs. requested length: %s"
                       % (len(tr.data), acc_dur))
                raise Exception
...
def get_time_cor(station_cl, station, time_UTCdatetime, return_invalid=0):
    # import ipdb; ipdb.set_trace()
    
    try:
        skew_us=station_cl.skew[station].astype(float)
    except Exception:
        print(f'No skew for {station_cl.name[station]}. Returning {return_invalid}')
        return(return_invalid)
    
    skew_s=skew_us/1.0e6
    # import ipdb; ipdb.set_trace()
    try:
        skew_date=UTCDateTime(station_cl.obs_end[station])
    except:
        print(f'Cannot find skew date for {station_cl.name[station]}. Returning {return_invalid}')
        return(return_invalid)
    try:    
        deploy_date=UTCDateTime(station_cl.obs_start[station])
    except:
        print(f'Cannot find deploy date for {station_cl.name[station]}. Returning {return_invalid}')
        return(return_invalid)
    # import ipdb; ipdb.set_trace()
    time_cor_s=skew_s*(time_UTCdatetime-deploy_date)/(skew_date-deploy_date)
    print('skew:',  skew_s, '    Time Correction: ', time_cor_s)
    return time_cor_s
...
def read_raw_obs(all_events, all_stations, inp_ffproc, ev, lancz_a=12):
    ...
    # counte = 0 
    # import ipdb; ipdb.set_trace()
    # print(len(all_stations.name), len(all_stations.skew))
    for cha in range(len(all_stations.name)):
        # print('Station: ', all_stations.name[cha])
        # cha_id = all_stations.name[cha].split('.')
        # real_name = '%s.%s.%s.%s' % (cha_id[0],
        #                              cha_id[1],
        #                              cha_id[2],
        ...
        try:
            ...
            if inp_ffproc.skew_correction:
                time_cor_s = get_time_cor(all_stations, cha, tr.stats.starttime)
                tr.stats.starttime = tr.stats.starttime + time_cor_s
            ...
            inv = all_stations.inv_path[cha]
            tr.attach_response(inv)
            tr = tr.remove_response(output=inp_ffproc.trace_unit, 
                                    pre_filt=inp_ffproc.trace_pre_filt,
            ...
            all_stations.real_start = np.append(all_stations.real_start,
                                    ...
                                    all_events.datetime[ev])
            # counte += 1
            # print(counte, len(all_stations.real_rdable))
        except Exception as e:
            cprint('StationClass.py', '[WARNING] [REAL]', bc.orange,
                   'Can not read %s, %s' % (real_data_path, e))
            # import ipdb; ipdb.set_trace()
...
def filter_stations(inp_ffproc, all_stations):
    ...
    if inp_ffproc.real_id_state == 0:
        ...
        indxs = np.multiply(indxs, indxs_ones.astype(bool))
...
class StationClass:
    ...
    def addsta(self, real_path, inv_path, name, lat, lon, dp, el, evla, evlo, obs_start, obs_end, skew):
        ...
        self.sta = np.append(self.sta, sta)
        ...
        self.inv_path = np.append(self.inv_path, inv_path)
        
        # when we have OBS data 
        self.obs_start = np.append(self.obs_start, obs_start)
        self.obs_end = np.append(self.obs_end, obs_end)
    ...
    # def get_time_cor(self, station, time_UTCdatetime, return_invalid=np.nan):
    #     # import ipdb; ipdb.set_trace()
    #     print('Class get time correction')
    #     try:
    #         skew_us=self.skew[station].astype(float)
    #     except Exception:
    #         print(f'No skew for {self.name[station]}. Returning {return_invalid}')
    #         return(return_invalid)
        
    #     skew_s=skew_us/1.0e6
        
    #     try:
    #         skew_date=self.endtime[station]
    #     except:
    #         print(f'Cannot find skew datefor {self.name[station]}. Returning {return_invalid}')
    #         return(return_invalid)
    #     try:    
    #         deploy_date=self.starttime[station]
    #     except:
    #         print(f'Cannot find deploy date for {self.name[station]}. Returning {return_invalid}')
    #         return(return_invalid)
    ...
    def rmsta(self, indxs):
        ...
        if len(self.inv_path) > 0:
            self.inv_path = self.inv_path[indxs]
        ...
        if len(self.obs_start) > 0:
            self.obs_start = self.obs_start[indxs]
        if len(self.obs_end) > 0:
            self.obs_end = self.obs_end[indxs]
        if len(self.skew) > 0:
            self.skew = self.skew[indxs]
